...
/// The frame the projector is aimed at.
final class CanvasPresetTests: XCTestCase {

    func testEveryPresetButCustomCarriesASize() {
    ...
    func testAKnownSizeIsRecognisedAndAnythingElseIsCustom() {
    ...
    func testTheShelfCoversPortraitAsWellAsLandscape() {
        // A projector turned on its side to light a doorway or a column is normal.
    ...
    func testATypedSizeIsAcceptedAndRounded() {
        let size = CanvasPreset.validate(width: 1920.4, height: 1080.6)
        XCTAssertEqual(size, CGSize(width: 1920, height: 1081))
    }

    /// A zero side gives a degenerate aspect ratio, and absurd numbers are a typo.
    func testAnUndrawableSizeIsRefused() {
        XCTAssertNil(CanvasPreset.validate(width: 0, height: 1080))
        XCTAssertNil(CanvasPreset.validate(width: 1920, height: 0))
        XCTAssertNil(CanvasPreset.validate(width: -1920, height: 1080))
        XCTAssertNil(CanvasPreset.validate(width: 1e9, height: 1080))
        XCTAssertNil(CanvasPreset.validate(width: .nan, height: 1080))
        XCTAssertNil(CanvasPreset.validate(width: .infinity, height: 1080))
    }
